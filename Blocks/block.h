...
public:
    ...
    virtual & std::vector<std::pair<int, int>> getPos(); // I think other Ethan needs this for display, basically return whats in blockPos. Uhm syntax is wrong but idk what its supposed to be (might also need to make blockPos public but that bad oop. We can return a copy of it tho and keep it protected)
